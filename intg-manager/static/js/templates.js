...
(function () {
  ...
  window.checkForFirmwareUpdate = async function () {
    ...
    try {
      const resp = await fetch('/api/diagnostics/firmware-update');
  ...
  window.fetchLogs = function () {
    ...
    if (window.htmx) htmx.ajax('GET', `/api/integration-logs/entries?${params.toString()}`, { target: '#log-entries', swap: 'innerHTML' });
  };
