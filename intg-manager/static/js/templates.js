...
(function () {
    ...
    window.powerOffRemote = () => postSystemAction('/api/system/power-off');
    ...
    window.onServiceChange = function () {
        ...
        if (selected.length) window.fetchLogs();
    };
    ...
    window.fetchLogs = function () {
        ...
        if (window.htmx) htmx.ajax('GET', `/api/integration-logs/entries?${params.toString()}`, { target: '#integration-log-entries', swap: 'innerHTML' });
    };
    window.downloadLogs = function () {
        const services = window.getSelectedServices();
        const priority = $('priority-select')?.value || '7';
        if (!services.length) { alert('Please select at least one service first'); return; }
        const url = `/api/integration-logs/download?service=${encodeURIComponent(services.join(','))}&priority=${encodeURIComponent(priority)}`;
    ...
    function initLogsPages() {
        ...
        const integrationAuto = $('auto-refresh-logs');
        if (integrationAuto && !integrationAuto.dataset.bound) {
            integrationAuto.dataset.bound = 'true';
            let interval = null;
            integrationAuto.addEventListener('change', function () {
                if (this.checked) interval = setInterval(window.fetchLogs, 5000);
    ...
    function initSystemMessages() {
        ...
        document.body.addEventListener('htmx:beforeRequest', function (event) {
            if (event.detail.elt?.id === 'refreshBtn') {
                event.detail.elt.querySelector('i')?.classList.add('fa-spin');
            }
        });
        document.body.addEventListener('htmx:afterRequest', function (event) {
            if (event.detail.elt?.id === 'refreshBtn') {
                event.detail.elt.querySelector('i')?.classList.remove('fa-spin');
                if (event.detail.xhr.status === 200) location.reload();
                else alert('Failed to refresh messages from GitHub. Please try again later.');
            }
        });
    }
    ...
    function initUpdatingPage() {
        ...
        setTimeout(function () {
            ...
            pingTarget.setAttribute('hx-on::after-request', "if(event.detail.xhr.status === 200 && event.detail.xhr.responseText.trim() === 'OK') { window.location.href = '/'; }");
    ...
    function initNotifications() {
        if (!$('notification-message') || window.__notificationsBound) return;
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        initSystemMessages();
        initUpdatingPage();
        if ($('firmware-check-btn')) window.checkForFirmwareUpdate();
    });
})();
