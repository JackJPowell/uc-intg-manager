...
import { ArchiveRestore, CircleCheck, Download, LockKeyhole, RefreshCw, Save, Server, Settings2, TriangleAlert, Upload } from 'lucide-react'
...
const sections: Array<{ title: string; detail: string; fields: Array<[keyof SettingsPayload['settings'], string, string]> }> = [
  { title: 'Manager availability', detail: 'Control how integration manager behaves when the Remote leaves its dock.', fields: [['shutdown_on_battery', 'Pause on battery', 'Stop manager operations while the Remote is undocked to conserve battery.']] },
  { title: 'Updates', detail: 'Choose the defaults when new integration versions arrive.', fields: [['auto_update', 'Automatically update integrations', 'Apply eligible releases in place while preserving existing configuration.' ], ['show_beta_releases', 'Show beta releases', 'Include pre-release versions in version selection.']] },
...
export function SettingsPage() {
  ...
  const save = useMutation({ mutationFn: api.saveSettings, onSuccess: data => { setValues(data); setSaveConfirmed(true) } })
  ...
  const restore = useMutation({ mutationFn: api.importBackups, onSuccess: () => {
  ...
  return <section className="settings-workspace">
    ...
    {saveConfirmed && <div className="notice success" role="status"><CircleCheck /> Settings saved. Changes take effect immediately.</div>}
    {restore.data && <div className="notice success" role="status"><CircleCheck /><span><strong>Backup imported.</strong> {restore.data.message}{restore.data.settingsRestored ? ' Imported settings are active now; no manager restart is needed.' : ' The backup did not contain settings for this remote.'}</span></div>}
