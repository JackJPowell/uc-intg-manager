...
export function LogsPage() {
  ...
  const clear = useMutation({ mutationFn: api.clearLogs, onSuccess: () => client.setQueryData(['logs'], []) })
  ...
  return <section className="manager-log-workspace"><header className="manager-log-hero"><div><p className="eyebrow">Manager activity</p><h1>Manager logs</h1><p>Local operational history for the Integration Manager.</p></div><div className="log-actions"><button className="secondary-action" type="button" onClick={() => logs.refetch()} disabled={logs.isFetching}><RefreshCw className={logs.isFetching ? 'spin' : ''} /> Refresh</button><button className="danger-action" type="button" disabled={clear.isPending} onClick={() => { if (window.confirm('Clear all manager log entries? This cannot be undone.')) clear.mutate() }}><Trash2 /> Clear logs</button></div></header>
