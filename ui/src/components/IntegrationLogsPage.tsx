...
const levelName = (entry: Record<string, unknown>) => String(entry.level ?? entry.priority ?? 'debug').toLowerCase()
...
const messageFor = (entry: Record<string, unknown>) => String(entry.message ?? entry.msg ?? entry.text ?? JSON.stringify(entry))
