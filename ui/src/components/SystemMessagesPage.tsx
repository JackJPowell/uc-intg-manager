...
export function SystemMessagesPage() {
  ...
  return <section className="page-section messages-workspace"><header className="page-heading messages-hero"><div><p className="eyebrow">Manager announcements</p><h1>System messages</h1><p>Release notices and operational updates from Integration Manager.</p></div><button className="refresh-button" type="button" onClick={() => refresh.mutate()} disabled={refresh.isPending}><RefreshCw className={refresh.isPending ? 'spin' : ''} /> Refresh</button></header>{(messages.isError || refresh.isError) && <div className="notice error"><TriangleAlert /> {(messages.error ?? refresh.error)?.message}</div>}
