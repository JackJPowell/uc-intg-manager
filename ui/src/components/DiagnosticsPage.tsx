...
import { api, type DiagnosticActivityResult, type IrCodeset } from '../lib/api'
...
function ScanResult({ result, empty, warning }: { result?: DiagnosticActivityResult; empty: string; warning: string }) {
  ...
  return <div className="diagnostics-results">{activities.map(([id, activity]) => <div className="diagnostics-activity" key={id}><div><strong>{activity.name}</strong><span>{activity.entities.length} {activity.entities.length === 1 ? 'entity' : 'entities'}</span></div><ul>{activity.entities.map((entity, index) => <li key={`${entity.id ?? entity.entity_id ?? index}`}><span>{entity.localized_name ?? String(entity.entity_id ?? entity.id ?? 'Unknown entity')}</span>{entity.integration?.localized_name && <small>{entity.integration.localized_name}</small>}</li>)}</ul></div>)}<p className="diagnostics-help"><ShieldAlert />{warning}</p></div>
}
...
export function DiagnosticsPage() {
  ...
  const firmware = useMutation({ mutationFn: api.checkFirmware })
  const codesets = useQuery({ queryKey: ['diagnostics', 'ir-codesets'], queryFn: api.orphanedIrCodesets })
  const removeCodeset = useMutation({ mutationFn: api.deleteIrCodeset, onSuccess: () => client.invalidateQueries({ queryKey: ['diagnostics', 'ir-codesets'] }) })
